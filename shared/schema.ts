...
export const payroll = pgTable("payroll", {
  ...
  dues_earned: decimal("dues_earned", { precision: 10, scale: 2 }).notNull().default("0"),
  // Source: 'calculated' = generated by system; 'migrated' = imported historical payroll
...
// Used for historical payroll: when generating payroll for a past month, salary (and category/accommodation
// when set) from the effective month are used so old data and old salary calculations are preserved.
export const employeeSalaryHistory = pgTable("employee_salary_history", {
  ...
  // Mid-month increment: from which day (1-31) this salary applies. Null/1 = whole month.
  effective_from_day: integer("effective_from_day"), // 1-31, null = from 1st
  // Historical accuracy: category and accommodation affect food allowance eligibility
  category: text("category").notNull().default("Direct"),
  accommodation: text("accommodation").notNull().default("Own"),
  // Source: 'system' = created in-app (snapshot/bulk); 'migrated' = imported historical data
  source: text("source").notNull().default("system"),
  created_at: timestamp("created_at").notNull().defaultNow(),
  notes: text("notes"), // Optional reason for change (e.g., "Annual increment", "Promotion", "Migrated")
});
